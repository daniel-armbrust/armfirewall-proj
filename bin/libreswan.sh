...
GLOBALS_FILE="${SCRIPT_DIR}/scripts/common/globals.sh"
...
if ! "${PYTHON_BIN}" -c 'import sys; raise SystemExit(0 if sys.version_info >= (3, 7) else 1)' >/dev/null 2>&1; then
    echo "error: ${PYTHON_BIN} must be Python 3.7 or newer." >&2
    exit 1
fi

exec "${PYTHON_BIN}" "${SCRIPT_DIR}/scripts/py/libreswan.py" "$@"
