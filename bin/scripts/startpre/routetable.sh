...
apply_policy_route() {
    ...
    IFS=$'\t' read -r id family table_id route_type destination gateway dev preferred_source metric scope protocol onlink <<< "$1"
...
apply_policy_rule() {
    ...
    IFS=$'\t' read -r id family priority source_addr destination_addr incoming_iface outgoing_iface fwmark fwmask tos dsfield ip_proto sport dport uid_range action table_id suppress_prefixlength suppress_ifgroup realms goto_priority <<< "$1"
    [[ -n "${id:-}" ]] || return 0
...
apply_policy_routing() {
    ...
    while IFS=$'\t' read -r _id table_id table_name; do
