...
apply_ipv4_router_forward_rules() {
    ...
    iptables -t filter -C FORWARD -i "$LAN_IFACE" -o "$WAN_IFACE" -m conntrack --ctstate NEW,ESTABLISHED,RELATED -j ACCEPT 2>/dev/null || \
    iptables -t filter -A FORWARD -i "$LAN_IFACE" -o "$WAN_IFACE" -m conntrack --ctstate NEW,ESTABLISHED,RELATED -j ACCEPT
...
apply_ipv6_router_forward_rules() {
    ...
    ip6tables -t filter -C FORWARD -i "$LAN_IFACE" -o "$WAN_IFACE" -m conntrack --ctstate NEW,ESTABLISHED,RELATED -j ACCEPT 2>/dev/null || \
    ip6tables -t filter -A FORWARD -i "$LAN_IFACE" -o "$WAN_IFACE" -m conntrack --ctstate NEW,ESTABLISHED,RELATED -j ACCEPT
...
record_router_forward_rules() {
    record_router_forward_rule "$IPV4_FILTER_RULES_DB" "0.0.0.0/0" "$LAN_IFACE" "$WAN_IFACE" 1 1 1
    record_router_forward_rule "$IPV4_FILTER_RULES_DB" "0.0.0.0/0" "$WAN_IFACE" "$LAN_IFACE" 0 1 1
    record_router_forward_rule "$IPV6_FILTER_RULES_DB" "::/0" "$LAN_IFACE" "$WAN_IFACE" 1 1 1
