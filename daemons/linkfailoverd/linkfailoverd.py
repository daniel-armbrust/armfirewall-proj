...
def verify_database() -> None:
    ...
    db.verify_database(LINKFAILOVER_DB_PATH)
...
def ping_once(link: Link, settings: dict[str, Any]) -> tuple[bool, float | None, str | None]:
    ...
    try:
        completed = run_command(command, timeout=timeout_seconds + 3, check=False)
...
def current_default_iface() -> str | None:
    ...
    result = run_command(["ip", "-4", "route", "show", "default"], timeout=5, check=False)
...
def remove_default_routes() -> None:
    ...
    for _ in range(20):
        ...
        delete = run_command(["ip", "-4", "route", "del", "default"], timeout=5, check=False)
...
def apply_default_route(link: Link) -> None:
    ...
    result = run_command(["ip", "-4", "route", "add", "default", "dev", link.iface], timeout=8, check=False)
