...
from .constants import COLLECT_INTERVAL_SECONDS, LATENCY_DB_PATH, LATENCY_DS, LOG_SOURCE
...
def sanitize_target_name(name: str) -> str:
    ...
    return rrd_safe_name(name.strip(), default="target")
...
def ensure_rrd(rrdtool: str, target: LatencyTarget) -> Path:
    ...
    if not rrd_needs_creation(rrdtool, rrd_path, LATENCY_DS):
        return rrd_path
