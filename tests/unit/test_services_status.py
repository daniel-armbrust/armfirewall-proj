...
from web.services import status as services_status
...
class SupervisorStatusParserTests(unittest.TestCase):
    ...
    def test_expected_service_statuses_marks_missing_services(self) -> None:
        ...
        self.assertFalse(by_name["armfirewall-dnsmasq"]["installed"])
        self.assertEqual(by_name["armfirewall-dnsmasq"]["state"], "NOT INSTALLED")
    ...
    def test_optional_service_statuses_include_squid_proxy(self) -> None:
        ...
        self.assertEqual(services[0]["name"], "armfirewall-squid")
        self.assertEqual(services[0]["display_name"], "SQUID Proxy")
        self.assertEqual(services[0]["state"], "NOT INSTALLED")
    ...
    def test_optional_service_install_is_queued(self) -> None:
        ...
        original_db_path = services_status.WORK_REQUEST_DB_PATH
        ddl_path = Path(__file__).resolve().parents[2] / "db" / "ddl" / "work-requests.ddl"
        try:
            with TemporaryDirectory() as tmpdir:
                tmp_db_path = Path(tmpdir) / "work-requests.db"
                services_status.WORK_REQUEST_DB_PATH = tmp_db_path
                with services_status.db.transaction(tmp_db_path, require_existing=False) as conn:
                    conn.executescript(ddl_path.read_text(encoding="utf-8"))

                result = services_status.install_optional_service("armfirewall-squid")

                self.assertEqual(result["status"], "queue")
                with services_status.db.connection(tmp_db_path) as conn:
                    row = services_status.db.fetch_one_on(
                        conn,
                        """
                    ...
                    self.assertIn("armfirewall-squid", row["payload_json"])
        finally:
            services_status.WORK_REQUEST_DB_PATH = original_db_path
    ...
    def test_optional_service_uninstall_is_queued(self) -> None:
        ...
        try:
            with TemporaryDirectory() as tmpdir:
                ...
                result = services_status.uninstall_optional_service("armfirewall-squid")
