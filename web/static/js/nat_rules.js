(function () {
    ...
    const originFilterSelects = Object.fromEntries(NAT_CHAINS.map((chain) => [chain, document.querySelector(`[data-nat-origin-filter='${chain}']`)]));
    ...
    function renderChain(chain, rules) {
        ...
        if (!visibleRules.length) {
            body.innerHTML = `
                <tr>
                    <td colspan="10">
                        <div class="terminal-empty"><span class="prompt">$</span><span>no ${familyLabel(activeFamily)}${activeOrigin === "user" ? " user defined" : ""} ${HF.escapeHtml(chain)} NAT rules</span></div>
                    </td>
                </tr>
            `;
            return;
