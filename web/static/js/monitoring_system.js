(() => {
    ...
    let currentPeriod = localStorage.getItem("armfw.monitoring.system.period") || "daily";
    let lastGraphs = [];
    ...
    async function refreshGraphs() {
        try {
            const response = await fetch("/api/monitoring/system", { cache: "no-store", credentials: "same-origin" });
            if (!response.ok) {
    ...
    periodButtons.forEach((button) => {
        button.addEventListener("click", () => {
            ...
            localStorage.setItem("armfw.monitoring.system.period", currentPeriod);
            updateAllGraphs();
        });
    });
