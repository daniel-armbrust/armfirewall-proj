(() => {
    ...
    const interfaceSelect = document.getElementById("network-interface-select");
    ...
    let currentPeriod = localStorage.getItem("armfw.monitoring.network.period") || "daily";
    let lastGraphs = [];
    let activeModalGraphId = "";
    let refreshToken = Date.now();
    let currentInterface = localStorage.getItem("armfw.monitoring.network.interface") || "";
    ...
    function latestUpdatedLabel() {
        ...
        lastGraphs.forEach((graph) => {
            if (graph.iface_safe_name !== currentInterface) {
                return;
            }
    ...
    function initializeInterfaceSelection() {
        ...
        const values = Array.from(interfaceSelect.options).map((option) => option.value);
        if (!currentInterface || !values.includes(currentInterface)) {
            currentInterface = values[0] || "";
        }
        interfaceSelect.value = currentInterface;
    }

    function setVisibleInterface() {
        cards.forEach((card) => {
            const visible = !currentInterface || card.dataset.ifaceName === currentInterface;
            card.hidden = !visible;
        });
    }
    ...
    async function refreshGraphs() {
        try {
            const response = await fetch("/api/monitoring/network", { cache: "no-store", credentials: "same-origin" });
            if (!response.ok) {
    ...
    periodButtons.forEach((button) => {
        button.addEventListener("click", () => {
            ...
            localStorage.setItem("armfw.monitoring.network.period", currentPeriod);
            updateAllGraphs();
        });
    });
    ...
    if (interfaceSelect) {
        interfaceSelect.addEventListener("change", () => {
            ...
            localStorage.setItem("armfw.monitoring.network.interface", currentInterface);
            updateAllGraphs();
        });
    }
